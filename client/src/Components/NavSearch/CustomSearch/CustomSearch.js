...
const clickedSearchHandler = async(e) => {
  ...
  try {
    if(expanded.renderedPage !== 1 ) {
      ...
      let renderHelper = 1
      const response = await fetch(`https://api.themoviedb.org/3/search/multi?api_key=b88a57406d9a87698d307358f3e4f4ab&language=en-US&query=${searchData}&page=${renderHelper}&include_adult=false`)
